...
function applyResponsiveStyles(): void {
    ...
    const headerP = document.querySelector<HTMLElement>('.header p');
    ...
    const paginationButtons = document.querySelectorAll<HTMLButtonElement>('.pagination button');

    if (!container || !headerH1 || !headerP || !controls) return;
    ...
    if (width <= 480) {
        ...
        if (newsGrid) newsGrid.style.gridTemplateColumns = '1fr';
        ...
        if (newsGrid) newsGrid.style.gridTemplateColumns = 'repeat(auto-fill, minmax(350px, 1fr))';
